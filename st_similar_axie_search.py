...
def check_input(_input, check_method=0):
    ...
    if check_method == 1:
        _input = str(_input).replace(",", "").replace(
            ".", "").replace("#", "").strip()
        if _input.isnumeric():
            return _input
        else:
...
def get_url(axie_id, filters=set()):
    ...
    axie_response = requests.request(
        "GET", axie_url, headers=headers, data=payload)
    ...
    search_parts = [
        f"parts={part}-{parts[part]}" for part in parts if part.title() not in filters]
    search_url = f"https://app.axieinfinity.com/marketplace/axies/?partTypes=Tail&auctionTypes=Sale&{'&'.join(search_parts)}" + "&classes=" + str(
        parts["class"])
...
def get_price_data(parts, filters=set()):
    ...
    ql_parts = ['horn-' + parts['horn'], 'eyes-' + parts['eyes'], 'ears-' + parts['ears'],
                'mouth-' + parts['mouth'], 'back-' + parts['back'], 'tail-' + parts['tail']]
    ql_combined = [{"Filter": f, "Parts": part}
                   for f, part in zip(ql_filter, ql_parts)]
    # Format the data to be put into the graphql payload
    part_ql_inject = [item['Parts']
                      for item in ql_combined if item['Filter'] not in filters]
    ...
    if response.status_code == 200:
        ...
        price_data = [{"id": id, "price": price}
                      for id, price in zip(ids, price)]
    else:
...
def multi_select(multi_axie_input):
    ...
    price_list = {axie: get_axie_data(axie)[1][0] if get_axie_data(
        axie)[1] else {'id': axie, 'price': '0'} for axie in multi_axie}
    undercut_axies = [axie for axie in price_list if axie !=
                      price_list[axie]['id'] and price_list[axie]['id'] != "0"]
...
def get_axies_from_address(address="1ed242dd-a96c-6f9f-9a64-775bfcf06d95", limit="20"):
    url = "https://api-gateway.skymavis.com/origin/v2/community/users/fighters?userID=" + \
        address + "&axieType=ronin&limit=" + str(limit)
...
def getEthPrice():
    ...
    payloadETH = {"operationName": "NewEthExchangeRate", "variables": {
    }, "query": "query NewEthExchangeRate {\n  exchangeRate {\n    eth {\n      usd\n      __typename\n    }\n    slp {\n      usd\n      __typename\n    }\n    ron {\n      usd\n      __typename\n    }\n    axs {\n      usd\n      __typename\n    }\n    usd {\n      usd\n      __typename\n    }\n    __typename\n  }\n}\n"}
    responseETH = requests.post(ql_endpoint, json=payloadETH)

    ql_dataETH = json.loads(responseETH.content.decode('utf-8'))
    resultsETH = ql_dataETH['data']['exchangeRate']
    eth = resultsETH['eth']['usd']
    return eth


def get_axie_images(axie_id_list):
    axies = [item.strip() for item in axie_id_list.split(",")]
    return {int(item): "https://assets.axieinfinity.com/axies/" + str(item) + "/axie/axie-full-transparent.png" for item in axies}
...
if st.sidebar.checkbox("**Multi Axie Select**"):
    multi_axie_input = st.text_input(
        "Input Multiple IDs in CSV Format", key="multiselect")
    if multi_axie_input:
        st.write(
            "Note: If the input IDs are not on sale they will register as being undercut, working on fix")
        ...
        axie_images = get_axie_images(multi_axie_input)
        if undercut_axies:
            for axie in undercut_axies:
                st.write(axie, "Cheaper Axies Available")
                st.image(axie_images[int(axie)], width=200)
                st.write("Lower Axie ID: ", price_list[axie]['id'])
                st.write("Price USD: ",
                    price_list[axie]['price'])
                st.write("Price Eth: ", float(
                    price_list[axie]['price']) / eth_price)
...
if st.sidebar.checkbox("Help"):
    ...
    st.sidebar.write(
        "The app provides a link to the marketplace for axies with the same parts as the entered axie")
    st.sidebar.write(
        "It also shows if there is a lower priced axie on the market than the on entered")
...
if st.sidebar.checkbox("User ID Help"):
    st.sidebar.write(
        "Find your User ID by Loading the Game and Clicking on your Portrat")
